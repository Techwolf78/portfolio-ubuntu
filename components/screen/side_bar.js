...
export default function SideBar(props) {
    ...
    return (
        <>
            <div className={(props.hide ? " -translate-x-full " : "") + " absolute transform duration-300 select-none z-40 left-0 top-8 h-[calc(100%-2rem)] pt-1 w-auto flex flex-col justify-start items-center border-black border-opacity-60 bg-black bg-opacity-50"}>
                {
                    (
            ...
            <div onMouseEnter={showSideBar} onMouseLeave={hideSideBar} className={"w-1 h-[calc(100%-2rem)] absolute top-8 left-0 bg-transparent z-50"}></div>
        </>
    )
}
